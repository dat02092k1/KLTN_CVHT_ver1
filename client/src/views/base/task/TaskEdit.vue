<template>
  <div
    class="task-form mb-4 px-2 mx-auto flex justify-center items-center relative"
  >
    <!-- <label class="block mb-2 text-sm" for="pretext-input">Học kỳ</label> -->
    <router-link
      class="absolute top-3 left-4"
      :to="{ path: '/consultant/task' }"
    >
    ...
    <form @submit.prevent="submitForm" class="add--course m-6 p-3 bg-[#fff]">
      ...
      <a-form-item name="assignedStudents" label="Sinh viên">
        <a-select
      ...
      <div>
        <div class="items-center px-1 py-1">Thời hạn:</div>
        <a-date-picker
      ...
      <div class="flex flex-col items-center my-2">
        <button class="flex p-2 rounded text-[#fff] bg-[#324f90]" type="submit">
          Lưu
        </button>
      </div>
    </form>
    ...
    <div class="w-[60%] h-[60%] flex flex-col items-center">
      <h3>Tỉ lệ sinh viên hoàn thành nhiệm vụ đã giao</h3>
...
export default {
  ...
  async mounted() {
    ...
    const completedStudents = this.task.assignedStudents.filter(
      (student) => student.isCompleted === true
    );

    const numCompletedStudents = completedStudents.length;
    const remainStudents =
      this.task.assignedStudents.length - numCompletedStudents;
    ...
    this.formState.studentAssign = this.useTask.tasks.assignedStudents.map(
      (student) => ({
